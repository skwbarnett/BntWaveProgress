...
@interface BntWaveView ()

@property (nonatomic, strong) CADisplayLink *waveDisplaylink;

@property (nonatomic, strong) CADisplayLink *waveDisplay;
...
- (void)setUp
{
    ...
    _secondWaveColor = [UIColor colorWithRed:0.06 green:0.70 blue:0.67 alpha:0.5];
    _firstWaveColor = [UIColor colorWithRed:0.31 green:0.72 blue:0.69 alpha:0.5];
    
    waveGrowth = GrowSpeed;
    waveSpeed = 0.1/M_PI;
...
- (void)creatWave
{
    ...
    if (_waveDisplay == nil) {
        _waveDisplay = [CADisplayLink displayLinkWithTarget:self selector:@selector(creatCurWave:)];
        [_waveDisplay addToRunLoop:[NSRunLoop mainRunLoop] forMode:NSRunLoopCommonModes];
    }
...
- (void)creatCurWave:(CADisplayLink *)display
{
    [self animateWave];
    offsetX += waveSpeed;
    curWaveY = 2 * waterWaveHeight * (1 -_percent);
    [self creatCurrentFirstWaveLayerPath];
    [self creatCurrentSecondWaveLayerPath];
}

-(void)creatCurrentFirstWaveLayerPath{
    NSLog(@"%@",@(curWaveY));
    ...
    for (float x = 0.0f; x <=  waterWaveWidth ; x++) {
        // 正弦波浪公式
        y = waveAmplitude * sin(waveCycle * x + offsetX) + curWaveY;
...
-(void)creatCurrentSecondWaveLayerPath{
    ...
    for (float x = 0.0f; x <=  waterWaveWidth ; x++) {
        // 余弦波浪公式
        y = waveAmplitude * cos(waveCycle * x + offsetX) + curWaveY;
        CGPathAddLineToPoint(path, nil, x, y);
    }
